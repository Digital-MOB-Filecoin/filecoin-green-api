...
const MapList = async function (req, res, next) {
    try {
        var result = await pool.query('SELECT * FROM fil_map_view_v9 ORDER BY storage_providers DESC;');
    ...
    try {
        ...
        query += `),
            ...
            SELECT
                ...
                SUM(total) AS power
            FROM fil_miners_data_view_country_v9
            WHERE (miner in (${miners.map(miner => `'${miner}'`).join(',')})) AND date::timestamp = date_trunc('day', now() - interval '3 day')
            GROUP BY miner
            )
            SELECT
                miners_data.miner, country, city, lat, long, power
