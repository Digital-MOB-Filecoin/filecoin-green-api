...
class TotalEmissionsModel {
    ...
    async NetworkQuery(params) {
        ...
        try {
                ...
                FROM (
                    SELECT
                        SUM(energy_use_kW_lower * (CAST(COALESCE(ef_value, un_value, ${avg_value}) AS decimal) )) as cumulative_emissions_lower,
                        SUM(energy_use_kW_estimate * (CAST(COALESCE(ef_value, un_value, ${avg_value}) AS decimal) )) as cumulative_emissions_estimate,
                        SUM(energy_use_kW_upper * (CAST(COALESCE(ef_value, un_value, ${avg_value}) AS decimal) )) as cumulative_emissions_upper,
                        date
