...
class TotalEmissionsModel {
    ...
    async NetworkQuery(params) {
        ...
        try {
                ...
                FROM (
                    SELECT
                        SUM(energy_use_kW_lower * (CAST(COALESCE(avg_wt_value, avg_un_value, ${avg_value}) AS decimal) / 1000)) as cumulative_emissions_lower,
                        SUM(energy_use_kW_estimate * (CAST(COALESCE(avg_wt_value, avg_un_value, ${avg_value}) AS decimal) / 1000)) as cumulative_emissions_estimate,
                        SUM(energy_use_kW_upper * (CAST(COALESCE(avg_wt_value, avg_un_value, ${avg_value}) AS decimal) / 1000)) as cumulative_emissions_upper,
                        date
