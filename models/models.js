...
} = require("./008_sealing_energy-model-v-1-0-2");
const {
  StorageEnergyModelv_1_0_2,
} = require("./009_storage_energy-model-v-1-0-2");
...
} = require("./012_total-stored-over-time-model");
const {
  TotalSealedStoredOverTimeModel,
} = require("./013_total-sealed_plus_stored-over-time-model");
...
} = require("./016_cumulative-energy-use-model");
const {
  RenewableEnergyRatioModel,
} = require("./017_renewable-energy-ratio-model.js");
...
} = require("./020_total-emissions-with-renewable-model.js");
const {
  TotalEmissionsWithRenewableFloorModel,
} = require("./021_total-emissions-with-renewable-floor-model.js");
const {
  MinersEmissionScoresModel,
} = require("./022_miners-emission-scores-model");
const {
  MinersConfidenceScoresModel,
} = require("./023_miners-confidence-scores-model");
const {
  MinersLocationScoresModel,
} = require("./024_miners-location-scores-model");
const { MinersGreenScoresModel } = require("./025_miners-green-scores-model");
const {
  EnergyPerTransactionModel,
} = require("./026_energy-per-transaction-model");
const {
  EmissionsPerTransactionModel,
} = require("./027_emissions-per-transaction-model");
const {
  MinersEnergyReShareModel,
} = require("./028_miners-energy-re-share-model");
...
let totalEmissionsWithRenewableModel = new TotalEmissionsWithRenewableModel(
  pool,
);
let totalEmissionsWithRenewableFloorModel =
  new TotalEmissionsWithRenewableFloorModel(pool);
...
let minersEnergyReShareModel = new MinersEnergyReShareModel(pool);
...
class Models {
  ...
  LoadModels() {
    ...
    this.Register(minersEnergyReShareModel); //Miners Energy Re-Share
  ...
  Register(model) {
    ...
    if (ValidModel(model)) {
      ...
      INFO(
        `[Models] register model ${model.Name()} , id : ${id} code_name : ${model.CodeName()}`,
      );
    } else {
